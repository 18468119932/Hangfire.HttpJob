...
namespace Hangfire.HttpJob.Server
{
    public class HttpJobDispatcher : IDashboardDispatcher
    {
        ...
        public string GetJobDetailSet(string key)
        {
            try
            {
               var detailJobSet= JobStorage.Current.GetConnection().GetAllEntriesFromHash($"recurring-job:{key}");
                return  JArray.Parse(JObject.Parse(detailJobSet["Job"])["Arguments"].ToString())[0].ToString();
            } 
            catch (Exception ex)
            {
                Logger.ErrorException("HttpJobDispatcher.GetJobDetailSet", ex);
                return null;
            }
        }
    }
